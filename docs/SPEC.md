...
| v3.6 | 2026-04-06 | `3be1a25` | P10+P11 프론트 연동 (인사이트 API 전환, 콘텐츠/SNS 신규, 월별요약 API 전환) |
| v3.7 | 2026-04-06 | — | P12 데이터 정합성 감사: 12개 이슈 발견, 원인분석 + 해결계획 수립 (코드 변경 없음, 계획 문서화만) |
| v3.8 | 2026-04-06 | `9fa16ad` | D4~D12 정합성 수정 일괄 적용 (GA4 중복제거, 공구분석 단일소스, fetchAll 전체확장, 미래날짜 필터, 전체기간 시작일 확장) |
| v3.9 | 2026-04-06 | `b9aa76f` | D4 퍼널 브랜드 필터 개선 (노출/유입 광고데이터 우선), Telegram 일별 리포트 cron (매일 10시 KST) |
| v4.0 | 2026-04-06 | `8ec127c` | 설정 일일입력 탭 — 데이터 수집 현황 패널 추가 (자동/수동 분류, 최신일자, ✅/⚠️ 상태) |
...
| v4.7 | 2026-04-10 | `604260a` | P11.2 Overview CSV 내보내기 (KPI + 트렌드 + 브랜드 + 채널) |
| v4.8 | 2026-04-13 | — | 밸런스랩 공구 매출 자동 동기화 수정 — `sync_all_v2.py` 섹션7 추가, `sync_db_to_datahub.py` 채널 필터 수정 |
...
#### ~~이슈 1. daily_funnel channel 컬럼 누락~~ ✅ 해결됨 (`7bcea98`, 2026-04-05)
...
**검증 체크리스트:**
- [x] DB: daily_funnel에 channel 컬럼 존재, unique (date, brand, channel)
- [x] DB: brand에 채널명 없음 (all, balancelab만)
- [x] DB: nutty+cafe24 merge 후 데이터 누락 없음
- [x] Data Hub 시트: Funnel 탭 channel 컬럼 정상
- [x] sync_all.py: GA4 → (all, cafe24) 정상 저장
- [x] settings API: smartstore/cafe24 퍼널 정상 저장
- [x] upload-coupang-funnel: (all, coupang) 정상 저장
- [x] funnel 페이지: 소스별 필터 정상 작동
- [x] Overview: 퍼널 데이터 정상 표시
- [x] missing-dates: 채널별 누락 감지 정상
...
#### ~~이슈 2. ga4_* 채널 광고비 중복 집계~~ ✅ 해결됨 (`daf48d3`, 2026-04-06)

**현상:** `daily_ad_spend`에 `ga4_Performance Max` 채널 행이 존재, 광고비 집계 시 Google P-Max와 중복 계산 (317행, 약 474만원).

**원인:** `sync_all.py`가 GA4 시트의 UTM 성과 데이터를 `daily_ad_spend`에 저장할 때 `channel = "ga4_Performance Max"` 로 저장. GA4는 퍼널 전용이어야 하는데 spend 값이 0이 아니었음.

**해결:** DB에서 `ga4_%` 행 전체 삭제. `ads/insights/monthly-summary` API에 `.not("channel","like","ga4_%")` 필터 추가 (dashboard/cron은 이미 처리 중이었음).

---

#### 이슈 3. product_sales unique constraint lineup 미포함 (우선순위: 중간)
...
#### 이슈 4. Vercel 공구 override 미작동 (우선순위: 낮음)
...
#### ✅ 이슈 5. 밸런스랩 공구 매출 미동기화 (해결 2026-04-13)

**현상:** 밸런스랩 신규 공구 이벤트 발생 시 `product_sales`에 데이터가 들어오지 않음. 4월 2~7일 공구 매출(약 2천~1천만원/일)이 DB에 없었음.

**원인:** `밸런스랩 매출 시트`의 col10(공구 매출)/col11(구매건수)을 읽는 스크립트가 존재하지 않았음. `sync_all_v2.py`는 `Sales` 시트만 읽고, `sync_sales_auto.py`는 밸런스랩 행 자체가 없음. 기존 공구 DB 데이터(공구_코루비 등)는 수동 입력 추정.

**해결:**
- `sync_all_v2.py` 섹션 7 추가: `밸런스랩 매출 시트` col10~12 → `product_sales` (brand=balancelab, channel=공구) 자동 동기화
- `sync_db_to_datahub.py`: 채널 필터 `startswith("공구_")` → `startswith("공구")` 수정 (plain `공구` 채널 포함)
- 소급 동기화: 40건 (2026-03-03 ~ 2026-04-07)

**데이터 소스 구조:**
```
밸런스랩 매출 시트 (Google Sheets)
  └ col0: DATE ("4월 7일 (화)" 형식)
  └ col10: 공구 매출
  └ col11: 공구 구매건수
  └ col12: 공구 구매자
     → sync_all_v2.py 섹션7
     → product_sales (brand=balancelab, channel=공구)
```

**주의:** 인플루언서별 공구 채널 분리(`공구_코루비` 등)는 지원 안 됨. 밸런스랩 매출 시트는 일별 공구 합계만 제공.
...
| **P7** | 데이터 모니터링 | 데이터 누락 알림 배너, 이벤트 마커, 싱크 버튼 | ⏳ 7.1~7.4 완료(`cf479a2`). 7.5(OKR/예산 시트 연동) 미구현 |
| **P8** | 브랜드 상세 분석 | 브랜드별 라인업/서브브랜드/옵션 분석, 공구 상세 | ✅ 완료 |
| **P9** | Meta 크리에이티브 | 소재별 성과, 소재 퍼널, 피로도 감지, 영상 미리보기 | ✅ API+기본UI 완료 |
| **P10** | Content/SNS + SEO | 콘텐츠 성과 분석, GSC 연동, 네이버 캠페인 | ✅ 완료 (`3be1a25`) |
| **P11** | 고도화 | AI 인사이트, 월별 요약, 보고서 내보내기, Telegram 알림 | ✅ 전체 완료 — 11.2 CSV 내보내기(`604260a`), 11.4 CAC vs ROAS(`cf479a2`), 11.5 키워드 사분면, 11.6 YTD 완료 |
...
> ✅ 이슈 1 해결 후 적용된 brand+channel 분리 구조 (`7bcea98`)

**brand + channel 이중 키 구조:**

| brand | channel | 의미 | 전체 탭 표시 | 브랜드 탭 표시 |
|-------|---------|------|-------------|--------------|
| `all` | `cafe24` | GA4(유입) + 카페24(전환) 합산 | ✅ | 전체/너티/아이언펫/사입 |
| `all` | `smartstore` | 스마트스토어 (너티/아이언펫/사입) | ✅ | 전체/너티/아이언펫/사입 |
| `balancelab` | `smartstore` | 밸런스랩 스마트스토어 | ❌ (전체에서 제외) | 밸런스랩 탭에서만 |
| `all` | `coupang` | 쿠팡 퍼널 | ✅ | 너티/사입 탭 |

> - GA4(자사몰 유입: sessions, impressions)와 카페24 수기입력(전환: cart_adds, signups, repurchases)은 같은 (date, all, cafe24) 행에 merge
> - brand="all"은 특정 브랜드가 아닌 해당 채널 전체를 의미
> - 확장 원칙: 브랜드가 새 채널 입점 시 설정 페이지에서만 추가, 코드 수정 0
