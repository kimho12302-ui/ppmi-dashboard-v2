...
| v3.0 | 2026-04-05 | — | v2 UI+시트+스크립트 심층 분석 10개 누락항목 반영 (UTM, 드릴다운, 사분면, DailyInputGuide 등) |
...
| Phase | 이름 | 핵심 | v2 대비 |
|-------|------|------|---------|
| **P6** | Dashboard API 강화 | brand 파라미터, KPI 서버 계산, 이상치 감지, 공구 분석 | v2 복구 |
| **P7** | 데이터 모니터링 | 데이터 누락 알림 배너, 이벤트 마커, 싱크 버튼 | v2 복구 |
| **P8** | 브랜드 상세 분석 | 브랜드별 라인업/서브브랜드/옵션 분석, 공구 상세 | v2 복구 |
| **P9** | Meta 크리에이티브 | 소재별 성과, 소재 퍼널, 피로도 감지, 영상 미리보기 | v2 복구 |
| **P10** | Content/SNS + SEO | 콘텐츠 성과 분석, GSC 연동, 네이버 캠페인 | v2 복구 |
| **P11** | 고도화 | AI 인사이트 고도화, 보고서 내보내기, Telegram 알림 | v2 복구 + 신규 |
...
**수정 파일:**
- `src/app/api/dashboard/route.ts` — 전면 리빌드
- `src/app/page.tsx` — KPI 카드에 서버 계산값 사용 + 드릴다운
